...
class UserController {
	...
	async signin(req, res, next) {
		try {
			...
			res.cookie('accesstoken', tokens.accessToken, { maxAge: 86400 * 1000, sameSite: "Lax", secure: true });
			res.cookie('refreshtoken', tokens.refreshToken, { maxAge: 30 * 86400 * 1000, httpOnly: true, sameSite: "Lax", secure: true });
			// userData = { ...userData };
			res.json(userData);
		} catch (err) {
			next(err)
	...
	async logout(req, res, next) {
		try {
			...
			res.cookie('accesstoken', "", { maxAge: 86400 * 1000, sameSite: "Lax", secure: true });
			res.cookie('refreshtoken', "", { maxAge: 86400 * 1000, httpOnly: true, sameSite: "Lax", secure: true });
