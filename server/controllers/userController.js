...
class UserController {
	...
	async signin(req, res, next) {
		try {
			...
			res.cookie('accesstoken', tokens.accessToken, { maxAge: 86400 * 1000, sameSite: true, secure: true });
			res.cookie('refreshtoken', tokens.refreshToken, { maxAge: 30 * 86400 * 1000, httpOnly: true, sameSite: true, secure: true });
			// userData = { ...userData };
			res.json(userData);
		} catch (err) {
			next(err)
